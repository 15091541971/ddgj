		...
		<script type="text/javascript" >
			$(function(){
					...
					jump:function(obj,first){
					   ...
					   var size = curr*10-10; 
					    if(!first){
					    location.href='<%=path%>/demand/selectDemand.do?fromId='+userId+
					    '&size='+size+'&page='+curr;
					   } 
					 }
					});
                ...
                //单个删除
				 $(".deleteSupply").click(function() {
					...
					layer.confirm('您确定要删除吗？', {
						...
						location.href='<%=path%>/demand/deleteDemand.do?fromId='+userId+'&demand.demand_id='+id;
					    layer.msg("删除成功");
					});
				});
               ...
               function ckeckAll(){
                 ...
                 $(".deleteSelected").click(function(){
					 ...
					 btn : [ '确定', '取消' ]},function(){
                      ...
                      location.href='/demand/deleteDemand.do?fromId='+userId+'&supply.status=0'+'&supply_ids='+vals;
					});
                 });
				...
				<div class="xsz_main">
					...
					<div class="xsz_nav" style="height:auto">
					   ...
					   <table   class="layui-table" lay-skin="line" width="100%"  style="text-align:center" cellspacing="0">
					      ...
					      <c:forEach var="demand" items="${demand.data}" varStatus="len">
					        <tr class="${demand.demand_id}">
					          <td class="checkTd"><input type="checkbox" value="${demand.demand_id}"  class="checkBox"></td>
					          <td>${demand.title}</td>
					          <td>${demand.price}￥</td>
					          <td>
					            <fmt:formatNumber value="${demand.count}" type="number"></fmt:formatNumber>
					          </td>
					          <td>
					            <fmt:formatDate value="${demand.createTime}" pattern="yyyy年MM月dd日HH点mm分ss秒"/>
					          </td>
					          <td class="test1">
					          <c:if test="${supply.status!=7 && supply.status!=1}">
				                <a target="BJXQ" href="${pageContext.request.contextPath}/demand/selectDemandById.do?demand_id=${demand.demand_id}"><i class="layui-icon">&#xe642;</i></a><!-- 编辑 -->
				                <a class="deleteSupply" href="javaScript:void(0)" id="${demand.demand_id}"><i class="layui-icon">&#xe640;</i></a><!-- 删除 -->
				                </c:if>
				                <a href="${pageContext.request.contextPath}/demand/ckeckDemand.do?Demand_id=${demand.demand_id}" ><i class="layui-icon" >&#xe60a;</i></a><!-- 查看 -->
					          </td>
					        </tr>
					      </c:forEach>
					   </table>
